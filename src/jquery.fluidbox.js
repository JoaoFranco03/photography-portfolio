      ...
      fbElements: function () {
        ...
        $fbGhost.css({
          ...
          top:
            $fbThumb.offset().top -
            $fbWrap.offset().top +
            parseInt($fbThumb.css("borderTopWidth")) +
            parseInt($fbThumb.css("paddingTop")),
          left:
            $fbThumb.offset().left -
            $fbWrap.offset().left +
            parseInt($fbThumb.css("borderLeftWidth")) +
            parseInt($fbThumb.css("paddingLeft")),
        });
      },
    },
  ...
  $.extend(Plugin.prototype, {
    init: function () {
      ...
      if (
        ...
        ($fb.children().is("img") ||
          ($fb.children().is("picture") && $fb.find("img").length === 1)) &&
        $fb.css("display") !== "none" &&
    ...
    open: function () {
      ...
      $fb.removeClass("fluidbox--closed").addClass("fluidbox--loading");
    ...
    compute: function () {
      ...
      if (viewportRatio > thumbRatio) {
        computedHeight =
          imgNatH < globalData.viewport.h
            ? imgNatH
            : globalData.viewport.h * fb.settings.viewportFill;
        imgScaleY = computedHeight / imgH;
        imgScaleX = (imgNatW * ((imgH * imgScaleY) / imgNatH)) / imgW;
        imgMinScale = imgScaleY;
      } else {
        computedWidth =
          imgNatW < globalData.viewport.w
            ? imgNatW
            : globalData.viewport.w * fb.settings.viewportFill;
      ...
      var offsetY =
          $w.scrollTop() -
          $fbThumb.offset().top +
          0.5 * (imgH * (imgMinScale - 1)) +
          0.5 * ($w.height() - imgH * imgMinScale),
        offsetX =
          0.5 * (imgW * (imgMinScale - 1)) +
          0.5 * ($w.width() - imgW * imgMinScale) -
          $fbThumb.offset().left,
        scale =
          parseInt(imgScaleX * 100) / 100 +
          "," +
          parseInt(imgScaleY * 100) / 100;

      // Apply styles to ghost and loader (if present)
      $fbGhost.css({
        transform:
          "translate(" +
          parseInt(offsetX * 100) / 100 +
          "px," +
          parseInt(offsetY * 100) / 100 +
          "px) scale(" +
          scale +
          ")",
    ...
    close: function (d) {
      ...
      if (
        fb.instanceData.state === null ||
        typeof fb.instanceData.state === typeof undefined ||
      ...
      $fb
        .removeClass(function (i, c) {
          return (
            c.match(/(^|\s)fluidbox--(opened|loaded|loading)+/g) || []
          ).join(" ");
        })
        .addClass("fluidbox--closed");
    ...
    bindListeners: function () {
      ...
      if ($.isFunction($.throttle)) {
        $w.on(
          "resize.fluidbox" + fb.instanceData.id,
          $.throttle(fb.settings.resizeThrottle, resizeFunction)
        );
      } else {
    ...
    if (options === undefined || typeof options === "object") {
      ...
      this.each(function () {
        ...
        if (
          ...
          returnVal = instance[options].apply(
            instance,
